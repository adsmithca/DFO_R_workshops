...
- Below we set $Z$ to 0.9 if year is equals 2009, otherwise $Z$ is 0.5
...
Alternatively, the `ifelse` function can be used

```{r}
for (i in seq_along(ages)[-1]) {
  for (j in seq_along(years)[-1]) {
    Z <- ifelse(years[j] == 2009, 0.9, 0.5) # ifelse(test, values if TRUE, values if FALSE)
    N[i, j] <- N[i - 1, j - 1] * exp(-Z)
  }
}
```
...
- Functions are a core part of R and the ability to make your own custom function allows you to add functionality to R to make your life easier
    - It is also posible to make the lives of others easier if you share your functions
...
- Here, `my_fun` is the name of your function
...
But note that this is a needless function since there is a build in funtion for substraction
