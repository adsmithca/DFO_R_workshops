...
## `mutate()`: Creating and altering exisiting variables {.build}
...
```{r mutate_demo, eval = TRUE, echo=TRUE, purl=TRUE}
#Let's say we want to just look at legal size (82.5 mm+) lobsters:
tidy_tags <- tidy_tags %>%
    mutate(LEGAL_SIZE = LENGTH > 82.5,
...
```{r mutate_demo2, eval = TRUE, echo=TRUE}
#Let's say we want to just look at legal size (82.5 mm+) lobsters:
tidy_tags <- tidy_tags %>%
    mutate(LEGAL_SIZE = LENGTH > 82.5,
           #We can also change an existing variable: 
